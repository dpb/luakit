...
static GtkWidget *
grid_add_entry(GtkWidget *grid, gint row, const gchar *label_text,
...
show_auth_dialog(LuakitAuthData *auth_data, const char *login, const char *password)
{
    ...
    gtk_box_pack_start(GTK_BOX(gtk_dialog_get_action_area(dialog)), hbox, TRUE, TRUE, 0);
    ...
    auth_data->login_entry = grid_add_entry(grid, 0, "Username:", login, NULL);
    auth_data->password_entry = grid_add_entry(grid, 1, "Password:", password, NULL);
    ...
    gtk_window_set_focus(window, auth_data->login_entry);
